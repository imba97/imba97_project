...
const mainTimer = setInterval(async function() {
  ...
  .then(singleUnreadRes => {
    ...
    if(singleUnreadRes.data.code !== 0 || (singleUnreadRes.data.data.unfollow_unread + singleUnreadRes.data.data.follow_unread) === 0) return;

    // 获取消息
    axios.get(`https://api.vc.bilibili.com/session_svr/v1/session_svr/new_sessions?begin_ts=${new Date().getTime() - 5000}000&build=0&mobi_app=web`, {
      headers: commonHeaders
    })
    .then(res => {
      if(res.data.code !== 0 || typeof res.data.data.session_list === 'undefined') return;
      ...
      _.forEach(res.data.data.session_list, item => {
        ...
        if(/梗百科 (.*)/.test(msg)) {
          ...
          axios.get(`https://bili.imba97.cn/ji.php?kw=${encodeURIComponent(reg[1])}`)
          .then(jiRes => {
            sendMsg(item.talker_id, jiRes.data.data.replace(/\\n\\n/, '\n'))
            readed(item)
            return
          })
        }
...
function readed(item) {
  axios.get(`https://api.vc.bilibili.com/session_svr/v1/session_svr/update_ack?talker_id=${item.talker_id}&session_type=${item.session_type}&ack_seqno=${item.last_msg.msg_seqno}&build=0&mobi_app=web&csrf_token=${csrf}&csrf=${csrf}`, {
    headers: commonHeaders
  })
  .then(updateRes => {
...
export function sendMsg(uid, content, type = 1) {
  ...
  formData.append('msg[content]', type === 1 ? `{"content":"${content}"}` : content);
