  ...
  .on('message', async (message) => {
    ...
    if (/^梗百科/.test(content)) {
      ...
      const response = await axios(
        `${baseConfig.ji}${encodeURIComponent(keyWord)}`
      )
    ...
    // 过滤表情 获取图片
    if (
      (/^[@a-z0-9]/.test(content) || /cdnmidimgurl/.test(content)) &&
      ...
      if (row && row.isSend === 1) {
        ...
        result = []
        result.push(
          `番名：${res.data.result[0].filename}\n第${
            res.data.result[0].episode
          }集\n时间：${Math.floor(res.data.result[0].from / 60)}分${Math.floor(
            res.data.result[0].from % 60
          )}秒`
        )
      ...
      if (row && row.isSend === 1) {
        ...
        const res = await axios.post(
          `https://aip.baidubce.com/rest/2.0/image-process/v1/colourize?access_token=${token}`,
          formData,
          config
        )
...
function getIsSend(username, code) {
  return new Promise((resolve, reject) => {
    initDB(function () {
      db.get(
        `SELECT isSend FROM order_info where name = ? and code = ?`,
        [username, code],
        function (err, row) {
          resolve(row)
        }
...
function setIsSend(username, code, val) {
  return new Promise((resolve, reject) => {
    initDB(function () {
      db.get(
        `SELECT * FROM order_info where name = ? and code = ?`,
        [username, code],
        function (err, row) {
          if (err) reject(err)
          // 如果没有则插入数据
          if (!row) {
            ...
            const date = new Date().getTime()
            db.run(
              `UPDATE order_info SET isSend = ?, date = ? where name = ? and code = ?`,
              [val, date, username, code]
            )
            resolve({
              name: username,
              code: code,
...
async function getToken() {
  return new Promise((resolve, reject) => {
    initDB(function () {
      db.get(
        `SELECT * FROM order_info where name = 'token'`,
        async function (err, row) {
          //                                               token 过期  存入时间 + 一个月 < 当前时间 - 一天 （预留一天的时间更新）
          if (
            !row ||
            (row && row.code === null) ||
            (row && row.date + 2592000 < new Date().getTime() / 1000 - 86400)
          ) {
            ...
            db.run(
              `INSERT INTO order_info VALUES ('token',?,?,?)`,
              [res.data.access_token, 1, new Date().getTime() / 1000],
              function (insertErr) {
                if (insertErr) reject(insertErr)
